...
void ControllerTest::runTest() {
  ...
  beginTest("out chan");
  expect(mm_eq(cc.process(MidiMessage::controllerEvent(JCH(0), 7, 127), 3),
               MidiMessage::controllerEvent(JCH(3), 7, 127)));

  beginTest("filter");
  cc.set_filtered(true);
  expect(mm_eq(cc.process(MidiMessage::controllerEvent(JCH(0), 7, 127), 0),
               EMPTY_MESSAGE));
  ...
  expect(mm_eq(cc.process(MidiMessage::controllerEvent(JCH(0), 7, 127), 0),
               MidiMessage::controllerEvent(JCH(0), 10, 127)));

  beginTest("limit");
  cc.set_translated_cc_num(7);
  cc.set_range(false, true, 1, 120, 1, 120);

  expect(mm_eq(cc.process(MidiMessage::controllerEvent(JCH(0), 7, 0), 0),
               EMPTY_MESSAGE));
  ...
  expect(mm_eq(cc.process(MidiMessage::controllerEvent(JCH(0), 7, 121), 0),
               EMPTY_MESSAGE));
